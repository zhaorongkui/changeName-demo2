...
import type { TreeNode } from '@/types/dataSandbox'
...
const initTreeData = () => {
  ...
  getContentsTopicList({type:2}).then((res) => {
    if(res.success) {
      treeData.value = res.data
      emit('getFirst',treeData.value[0].name)
  ...
  // treeData.value = formatTagTree(tagMockList)
}
