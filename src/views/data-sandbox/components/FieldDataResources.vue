<template>
  <div class="content-main">
    ...
    <div v-if="activeTab === 'detail'" class="data-table-container">
      ...
      <el-table :data="tableData" class="table-content" border v-loading="isLoading" max-height="420px"
        ...
        <el-table-column label="是否脱敏" min-width="150" prop="desensitizationLevel" align="center"
          v-if="routePageType === 'review'">
          <template #default="{ row }">
            <el-switch v-model="row.delivery" @change="changeItem(row)"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="脱敏级别" min-width="150" prop="desensitizationLevel" align="center"
          v-if="routePageType === 'review'">
          <template #default="{ row }">
            <el-select v-model="row.desensitizationLevel" class="m-2" size="large" @change="changeItem(row, 'level')"
    ...
    <div v-if="activeTab === 'preview'" class="data-table-container">
      <el-table :data="tableDataPreview" class="table-content" border v-loading="loading" max-height="450px"
        :header-cell-style="{ background: '#f5f7fa', color: '#606266', fontWeight: 'bold' }">
        <el-table-column type="index" label="序号" width="60" />
...
import { updataDesensitizeField } from '@/api/backStageManage.ts'
...
// 脱敏接口提交
const changeItem = (item, level) => {
  // console.log(item);
  let params = {
    datasetId: props.currentDatasetDetail.datasetsId,
    bizObjId: queryData.value.tags,
    fieldId: item.id,
    desensitize: ''
  }
  // 切换脱敏级别
  if (level && level == 'level') {
    if (item.desensitizationLevel == '2') {
      params.desensitize = '2'
    } else {
      params.desensitize = '1'
    }
  } else {
    // 切换是否脱敏
    if (!item.delivery) {
      params.desensitize = '0'
      item.desensitizationLevel = ''
    } else {
      params.desensitize = '1'
      item.desensitizationLevel = '1'
    }
  }
  updataDesensitizeField(params).then(res => {
    if (res.success) {
...
// 获取字段信息
const getFieldData = () => {
  console.log(787887878);
  tableData.value = []
  isLoading.value = true
  if (!props.pageType) {
    console.log(props.currentDatasetDetail, 'props.currentDatasetDetail.id')
  ...
  } else if (props.pageType && (props.pageType == 'catalog' || props.pageType == 'application')) {
    getBusinessDetail({
      pageNum: tableConfig.value.paginationConfig.currentPage,
      pageSize: tableConfig.value.paginationConfig.pageSize,
      keyWord: searchContent.value,
      dataListId: queryData.value.tags
    }).then(res => {
      if (res.success) {
        tableData.value = res.data.records
        tableConfig.value.paginationConfig.total = res.data.total ? res.data.total : 0
...
const getPreViewData = () => {
  console.log(232323232, props.pageType)
  loading.value = true
  getDatasetDetailPreview({
    ...
    objectId: queryData.value.tags,
    datasetsId: props.currentDatasetDetail.datasetsId,
    kind: props.pageType && props.pageType == 'catalog' ? 2 : (props.pageType == 'application' ? 3 : 1),  //1:数据集广场 2：数据目录
  }).then(res => {
    if (res.success) {
...
onMounted(() => {
  nextTick(() => {
    if (props.pageType && (props.pageType == 'catalog' || props.pageType == 'application')) {
