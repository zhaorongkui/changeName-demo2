...
import {  getDatasetDetailPreview, getBusinessDetail } from '@/api/dataset.ts'
...
const handleTabChange = (pane: TabPaneName) => {
  ...
  console.log(activeTab.value, 'activeTab==999', pane)
  if (pane == 'detail') {
    getFieldData()
  } else {
    getPreViewData()
...
const getFieldData = () => {
  ...
  getBusinessDetail({
    ...
    pageSize: tableConfig.value.paginationConfig.pageSize,
    keyWord: searchContent.value,
    dataListId: datasetStore.currentCoreObjectID
  }).then(res => {
    if (res.success) {
      tableData.value = res.data.records
      tableConfig.value.paginationConfig.total = res.data.total ? res.data.total : 0
...
const getPreViewData = () => {
  ...
  getDatasetDetailPreview({
    ...
    objectId: queryData.value.tags,
    datasetsId: !props.pageType ? props.currentDatasetDetail.datasetsId : null,
